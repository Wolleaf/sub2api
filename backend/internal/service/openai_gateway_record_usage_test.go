...
func TestOpenAIGatewayServiceRecordUsage_CodexDefaultEchoKeepsFastBilling(t *testing.T) {
	for _, accountType := range []string{AccountTypeOAuth, AccountTypeSetupToken} {
		t.Run(accountType, func(t *testing.T) {
			...
			require.NoError(t, calcErr)
			expectedRatio := openAIModelFastPricingRatio("gpt-5.6-sol")
			if accountType == AccountTypeOAuth {
				// Only a refreshable OpenAI OAuth credential uses our custom
				// 2.5x contract; setup-token keeps upstream's 2x Fast price.
				expectedRatio = openAIOAuthFastCostMultiplier
			}
			require.InDelta(t, standardCost.TotalCost*expectedRatio, usageRepo.lastLog.TotalCost, 1e-10)
		})
	}
}
...
func TestOpenAIGatewayServiceRecordUsage_ShadowUsesParentCredentialTierContract(t *testing.T) {
	...
	standardCost, calcErr := svc.billingService.CalculateCost("gpt-5.6-sol", tokens, 1.0)
	require.NoError(t, calcErr)
	require.InDelta(t, standardCost.TotalCost*openAIOAuthFastCostMultiplier, usageRepo.lastLog.TotalCost, 1e-10)
}
...
func TestOpenAIGatewayServiceRecordUsage_FreeOpenAIFastChargesStandard(t *testing.T) {
	...
	// OAuth Fast keeps this fork's 2.5x upstream cost, while FreeOpenAIFast
	// still bills the user at the group's discounted Standard price.
	require.InDelta(t, standardTotal*openAIOAuthFastCostMultiplier, usageRepo.lastLog.TotalCost, 1e-10)
