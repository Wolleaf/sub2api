	...
	if responseModel := responseModelBillingDeclaration(
		...
		if identified, responseChannelPriced := s.hasIdentifiedOpenAIResponsePricing(ctx, responseModel, apiKey); identified {
			...
			responseCost, responseErr := s.calculateOpenAIRecordUsageCost(
				ctx, result, apiKey, billingAccount, responseModels, multiplier, imageMultiplier,
...
func shouldApplyOpenAIOAuthFastPricing(billingAccount *Account, billingModel, serviceTier string) bool {
	...
	return isOpenAIGPT6AstraModel(normalizedModel) || isOpenAIGPT56Model(normalizedModel) || normalizedModel == "gpt-5.5"
}
