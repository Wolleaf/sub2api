	...
	if apiKey.HasEnforcedRateLimits() && h.apiKeyService != nil {
		...
		if err == nil && rateLimitData != nil {
			...
			if limit := apiKey.EffectiveUpstreamWeeklyLimitPercent(); limit > 0 {
				share := gin.H{
					"limit": limit, "used": rateLimitData.UpstreamWeeklyUsagePercent,
					"remaining": max(0, limit-rateLimitData.UpstreamWeeklyUsagePercent),
					"unit":      "percent", "estimated": true,
					"window_start": rateLimitData.UpstreamWeeklyWindowStart,
					"observed_at":  rateLimitData.UpstreamWeeklyObservedAt,
				}
				if rateLimitData.UpstreamWeeklyWindowStart != nil {
					share["reset_at"] = rateLimitData.UpstreamWeeklyWindowStart.Add(service.RateLimitWindow7d)
				}
				resp["upstream_weekly_quota"] = share
				resp["remaining"] = share["remaining"]
				resp["unit"] = "percent"
			}
