...
type Group struct {
	...
	// 是否临时豁免本分组 API Key 的 7 天额度检查
	WeeklyRateLimitBypassEnabled bool `json:"weekly_rate_limit_bypass_enabled,omitempty"`
	// 启用豁免时观测到的 OpenAI 上游周窗口起点
...
func (*Group) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case group.FieldCreatedAt, group.FieldUpdatedAt, group.FieldDeletedAt, group.FieldWeeklyRateLimitBypassWindowStart:
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldWeeklyRateLimitBypassEnabled:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field weekly_rate_limit_bypass_enabled", values[i])
			} else if value.Valid {
				_m.WeeklyRateLimitBypassEnabled = value.Bool
			}
		case group.FieldWeeklyRateLimitBypassWindowStart:
			if value, ok := values[i].(*sql.NullTime); !ok {
				return fmt.Errorf("unexpected type %T for field weekly_rate_limit_bypass_window_start", values[i])
			} else if value.Valid {
				_m.WeeklyRateLimitBypassWindowStart = new(time.Time)
				*_m.WeeklyRateLimitBypassWindowStart = value.Time
			}
		default:
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("weekly_rate_limit_bypass_enabled=")
	builder.WriteString(fmt.Sprintf("%v", _m.WeeklyRateLimitBypassEnabled))
	builder.WriteString(", ")
	if v := _m.WeeklyRateLimitBypassWindowStart; v != nil {
		builder.WriteString("weekly_rate_limit_bypass_window_start=")
		builder.WriteString(v.Format(time.ANSIC))
	}
	builder.WriteByte(')')
